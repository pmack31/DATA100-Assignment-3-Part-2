...
write_parquet(ice_extent_yearly, sink = "ice_extent_yearly.parquet")
```
...
aware_pop <- inner_join(
    x = select(pop_data, country, total_population = SP.POP.TOTL),
    y = select(aware_fixed_country_match, -aware_base),
    by = join_by(country)
)
